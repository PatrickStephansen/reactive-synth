...
readdirAsync('./src/assets/audio-worklet-processors').then(files => {
  ...
  const bitcrusherWorkletFile = files.find(f => /bitcrusher.*\.js$/.test(f));
  const inverseGainWasmFile = files.find(f => /reactive_synth_inverse_gain.*\.wasm$/.test(f));
  const inverseGainWorkletFile = files.find(f => /inverse-gain.*\.js$/.test(f));
  ...
  console.log(`export const bitcrusherWorkletUrl = '${baseUrl}${bitcrusherWorkletFile}';`);
  console.log(`export const inverseGainWasmUrl = '${baseUrl}${inverseGainWasmFile}';`);
  console.log(`export const inverseGainWorkletUrl = '${baseUrl}${inverseGainWorkletFile}';`);
}, console.error);
